...
def main() -> None:
    ...
    required = {"YEAR", "MONTH", "CPSIDP", "OCC"}
    ...
    db.execute(
        "CREATE TABLE observations (pid TEXT, year INTEGER, month INTEGER, occ TEXT, "
        "PRIMARY KEY (pid, year, month, occ))"
    )

    rows = 0
    months: set[tuple[int, int]] = set()
    batch: list[tuple[str, int, int, str]] = []
    with gzip.open(DAT_PATH, "rt", encoding="utf-8") as source:
        for line in source:
            year = int(field(line, "YEAR"))
            month = int(field(line, "MONTH"))
            batch.append((field(line, "CPSIDP"), year, month, field(line, "OCC")))
            months.add((year, month))
            rows += 1
            if len(batch) >= 20_000:
                db.executemany("INSERT OR IGNORE INTO observations VALUES (?, ?, ?, ?)", batch)
                batch.clear()
    if batch:
        db.executemany("INSERT OR IGNORE INTO observations VALUES (?, ?, ?, ?)", batch)
    db.commit()
    ...
    edges = db.execute(
        f"SELECT COUNT(DISTINCT a.occ || ':' || b.occ) {join}"
    ).fetchone()[0]
    ...
    print(f"distinct_directed_edges={edges}")
